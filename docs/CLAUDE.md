...
### データベースマイグレーションは `migrate dev` を使う
- **ルール**: スキーマ変更時は必ず以下で行う。
  ```bash
  cd apps/web
  npx prisma migrate dev --name <migration_name>
  ```
- **理由**: `migrate dev` はマイグレーション履歴（`prisma/migrations/` と `_prisma_migrations` テーブル）を正しく作成・記録し、Prisma Client も再生成する。手書きSQLや `migrate deploy` だけで済ませると**履歴が残らず**、後からスキーマの差分追跡・チーム同期ができなくなる。
- **`migrate deploy` の用途**: 本番DBへ「既存の」マイグレーションを適用するときだけ使う（履歴の新規作成はしない）。
- **やってはいけないこと**: 開発DBに対して `db push` や手書きSQLでスキーマだけ変えて履歴を残さない、というやり方。
- **日付**: 2026-06-25
